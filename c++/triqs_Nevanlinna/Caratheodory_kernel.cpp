...
namespace triqs_Nevanlinna {
  ...
  nda::vector<double> Caratheodory_kernel::pick_eigenvalues() const {
    ...
    auto N    = _data.shape(1);
    auto Pick = matrix_cplx_mpt(nw * N, nw * N);
    auto id   = matrix_cplx_mpt::Identity(N, N);
    auto One  = complex_mpt{1., 0.};
    auto I    = complex_mpt{0., 1.};
    for (int i = 0; i < nw; i++) {
      for (int j = 0; j < nw; j++) { Pick.block(i * N, j * N, N, N) = (id - _data(i).adjoint() * _data(j)) / (One - std::conj(_mesh(i)) * _mesh(j)); }
    }
    auto eigenvalues      = Pick.eigenvalues();
    auto pick_eigenvalues = nda::vector<double>(N);
    std::transform(eigenvalues.begin(), eigenvalues.end(), pick_eigenvalues.begin(),
                   [](const complex_mpt &r) { return r.real().convert_to<double>(); });
